angular.module('itunes').controller('mainCtrl', function($scope, itunesService){
  ...
  //The value is 'songData'. That means ng-grid is looking for songData on $scope and is putting whatever songData is 
  //into the grid.
  //This means when you make your iTunes request, you'll need to get back the information, parse it accordingly, then 
  //set it to songData on the scope -> $scope.songData = ...
  $scope.gridOptions = { 
      data: 'songData',
      height: '110px',
  ...
  //*remember, that method should be expecting an artist name. The artist name is coming from the input box on index.html, head over there and check if that input box is tied 
  ...
  $scope.getSongData = function(artist) {
    ...
    var getSongDataPromise = itunesService.getArtist(artist);
    getSongDataPromise.then(function(response) {
  ...
  //What you need to do now is sort the data you got back. This sounds like a great job for a service! 
  //Head back to your itunesService and complete the last instructions there. 
